...
import { AlertController } from 'ionic-angular';
 ...
 export class BuyTravelPassPage {
	...
	initialiseBundle(){
		this.bundleSaveTickets.child_ticket_quantity = this.arraycount[0];
 		this.bundleSaveTickets.child_ticket_amount = this.totalCost[0];

 		this.bundleSaveTickets.adult_ticket_quantity = this.arraycount[1];
 		this.bundleSaveTickets.adult_ticket_amount = this.totalCost[1];

 		this.bundleSaveTickets.family_ticket_quantity = this.arraycount[2];
 	...
 	savePressed(){
		if((this.arraycount[0] == 0) && (this.arraycount[1] == 0) && (this.arraycount[2] == 0)){
 			this.presentAlert();
 		}	
 		else{
